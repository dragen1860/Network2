...
__all__ = ['Bottleneck', 'repnet_deep', 'repnet_sim']
...
class Bottleneck(nn.Module):
	...
	def __init__(self, inplanes, planes, stride=1, downsample=None):
		...
		self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
		                       padding=1, bias=False)
		self.bn2 = nn.BatchNorm2d(planes)
		self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False) # 4 -> 1
		self.bn3 = nn.BatchNorm2d(planes * 4) # 4->1
...
def repnet_sim(pretrained=False, **kwargs):
	"""Constructs a ResNet-Mini-Imagenet model.
	...
	model = ResNet_sim(BasicBlock, [1, 1, 1], **kwargs)
	return model